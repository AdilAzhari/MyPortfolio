...
const EntryCard: React.FC<{ entry: Entry }> = ({ entry }) => {
  const { period, title, subtitle, href, description, tags, links, thumb } = entry;
  ...
  return (
    <div className="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
      ...
      <div className="z-10 sm:col-span-6 sm:order-2">
        ...
        {thumb && <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500">{period}</p>}
